...
static int getBLEAddress(){
    ...
    if ( (fp = fopen("../exports/bleaddresses.txt","r+")) == NULL){
...
int bleScan(blescan_t* bleData)
{
    ...
    if ( device < 0 ) { 
        perror("Failed to open HCI device. Is Bluetooth turned on?");
        return 0; 
    }
    ...
    if ( ret < 0 ) {
        ...
        perror("Failed to set scan parameters data. Might have to run with sudo. ");
        return 0;
    }
