...
void scanWlan(int argc, char **argv, int skfd, Position_t position) {
	...
	/* WLAN Scan */
	if(strcmp(argv[4], "Wlan") == 0 || strcmp(argv[4], "BleAndWlan") == 0) {

		openFile("database_final.txt", "a+");
		...
		int i;
		for (i = 0; i < sizeof(retScanData)/sizeof(retScanData[0]); i++) {
